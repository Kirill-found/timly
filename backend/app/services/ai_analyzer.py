...
## ЗАПРЕЩЕННЫЕ ФРАЗЫ (КРИТИЧЕСКИ ВАЖНО)
НИКОГДА не используй в reasoning и summary_one_line:
- "Кандидат имеет опыт..."
- "Кандидат обладает..."
- "Опыт работы соответствует..."
- "Навыки соответствуют..."
- "Подходит для позиции..."

ВМЕСТО ШАБЛОНОВ пиши КОНКРЕТНО:
- "7 лет в продажах, руководитель в Сбере"
- "Знает Python, SQL, Power BI — 3 из 4 требований"
- "Вырос от менеджера до директора за 4 года"
...
{{
    ...
    "experience_years": <стаж в годах>,
    "last_position": "<последняя должность>",
    "last_company": "<последняя компания>",
    "candidate_salary": <ЗП ожидания или 0>,
    "matching_skills": ["навык1 совпадает", "навык2"],
    "skill_gaps": ["навык которого нет", "навык2"],
    "strengths": ["сильная сторона", "сторона2", "сторона3"],
    "weaknesses": ["слабая сторона", "сторона2", "сторона3"],
    "red_flags": ["флаг или пустой массив"],
    "career_trajectory": "<growth/stable/decline/start>",
    "recommendation": "<hire/interview/maybe/reject>",
    "reasoning": "ФАКТЫ с цифрами и компаниями",
    "summary_one_line": "10-15 слов для скрининга"
}}
...
### matching_skills и skill_gaps:
- matching_skills: навыки из вакансии которые ЕСТЬ в резюме
- skill_gaps: навыки из вакансии которых НЕТ в резюме
- Если всё совпадает — skill_gaps = []

### career_trajectory:
- "growth" — повышение в должности
- "stable" — работа на одном уровне
- "decline" — понижение
- "start" — начало карьеры (<2 лет)

### reasoning (ОБЯЗАТЕЛЬНО):
ПЛОХО: "Кандидат имеет релевантный опыт работы"
ХОРОШО: "8 лет в IT-продажах, вырос до руководителя. Нет Bitrix"

### summary_one_line:
ПЛОХО: "Кандидат подходит для позиции"
ХОРОШО: "Senior Python, 6 лет, ex-Яндекс, +30%"

## ВАЖНО:
- Указывай ТОЛЬКО факты из резюме
- НЕ придумывай данные
- Фокусируйся на измеримых результатах
- strengths и weaknesses — РОВНО по 3 элемента
"""
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Ты эксперт HR аналитик. Отвечай ТОЛЬКО в JSON. Используй конкретные факты и цифры."
                    },
                    {
                        "role": "user",
                        "content": prompt
                ...
                temperature=0.2,  # Меньше творчества
                max_tokens=1200,  # Больше для новых полей
